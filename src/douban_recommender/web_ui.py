...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .image-resilience { border-color:rgba(34,211,238,.28); background:rgba(34,211,238,.07); }
    .resilience-card { display:grid; gap:10px; border:1px solid rgba(34,211,238,.18); border-radius:18px; padding:14px; margin-top:12px; background:rgba(2,6,23,.38); }
    ...
    .person-card { border:1px solid var(--line); border-radius:20px; padding:12px; background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.04)); color:var(--text); text-align:left; min-height:160px; }
...
<script>
const state = { step:1, items:[], ratedItems:[], counts:{}, errors:[], diagnostics:[], recommendations:[], visibleRecommendations:[], sections:[], activeSection:'全部', heroIndex:0, heroBySection:{}, ratingsCsv:'', candidatesCsv:'', profile:null, lastCounts:{} };
...
function renderCrawlerPanel() { $('controlPanel').innerHTML = `<h2>第一步：连接豆瓣</h2><p class="hint">公开页够用就不填 Cookie；如果抓成 0 / 0，再粘贴 Cookie 并查看同步诊断。</p><div class="story-panel"><b>全站都已按私人影库工作流重做</b><p class="hint">同步、口味、推荐、详情和空状态会共享同一套电影级视觉系统，不再只优化推荐列表。</p></div><label>豆瓣用户 ID 或主页链接</label><input id="doubanUser" placeholder="https://www.douban.com/people/你的ID/"><label>Cookie（可选）</label><textarea id="doubanCookie" placeholder="公开数据够用就不用填"></textarea><div class="row"><div><label>期望看过</label><input id="expectedCollect" type="number" value="242"></div><div><label>期望想看</label><input id="expectedWish" type="number" value="34"></div></div><label>最多抓取页数</label><input id="maxPages" type="number" min="1" max="200" value="60"><label><input id="includeWish" type="checkbox" checked> 同步想看</label>${renderCookieGuide()}${imageResilienceGuide()}<details><summary>没有抓取数据？粘贴 CSV</summary><label>评分 CSV</label><textarea id="ratingsCsv" placeholder="title,my_rating,media_type,genres,tags">${esc(state.ratingsCsv)}</textarea><label>候选 CSV</label><textarea id="candidatesCsv" placeholder="title,media_type,douban_rating,genres,tags">${esc(state.candidatesCsv)}</textarea><button class="ghost" onclick="useCsvInputs()">使用 CSV 继续</button></details><div class="quick-actions"><button onclick="syncDouban()">同步豆瓣</button><button class="ghost" onclick="renderTastePanel()">下一步：确认口味</button><button class="ghost" onclick="clearCache()">清空缓存</button></div><div id="status" class="hint"></div>`; renderCrawlSummary(); }
...
function peopleCarousel(r) { const people = []; recArray(r,'directors').slice(0,4).forEach(name => people.push({name, role:'导演'})); recArray(r,'casts').slice(0,8).forEach(name => people.push({name, role:'主演'})); if (!people.length) return `<p class="hint">人物资料待补全。</p>`; return `<div class="people-carousel">${people.map(person => { const initials = String(person.name || '?').trim().slice(0,2).toUpperCase(); const encoded = encodeURIComponent(person.name || ''); return `<button class="person-card" onclick="filterByPerson('${encoded}')"><span class="avatar">${esc(initials)}</span><b>${esc(person.name)}</b><small>${esc(person.role)} · 点击查看 TA 参与的相关推荐</small></button>`; }).join('')}</div>`; }
