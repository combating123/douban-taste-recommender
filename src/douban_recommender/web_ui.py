...
function persistCrawlerControls() { const prefs = saveUserPrefs(); const cookieValue = normalizeCookieInput($('doubanCookie')?.value || ''); if ($('doubanCookie') && $('doubanCookie').value.trim() !== cookieValue) $('doubanCookie').value = cookieValue; if (prefs.rememberCookieSession && cookieValue) sessionStorage.setItem(COOKIE_SESSION_KEY, cookieValue); if (!prefs.rememberCookieSession) sessionStorage.removeItem(COOKIE_SESSION_KEY); return prefs; }
...
function normalizeCookieInput(value) {
  ...
  const idx = lines.findIndex(line => /^\s*cookie\s*:/i.test(line));
  if (idx >= 0) {
    const parts = [lines[idx].replace(/^\s*cookie\s*:\s*/i, '').trim()];
    for (let i = idx + 1; i < lines.length; i += 1) {
      const line = lines[i];
      if (/^\s/.test(line) && !/^\s*[\w-]+\s*:/i.test(line.trim())) parts.push(line.trim());
      else break;
    }
    return parts.filter(Boolean).join(' ').replace(/\s*;\s*/g, '; ').trim();
  }
  return text.replace(/^\s*cookie\s*:\s*/i, '').replace(/\n+/g, ' ').replace(/\s*;\s*/g, '; ').trim();
}
...
function setCookieBoxValue(value, message='已导入 Cookie，并自动启用本次会话记忆。') { const clean = normalizeCookieInput(value); const box = $('doubanCookie'); if (box) box.value = clean; if ($('rememberCookieSession')) $('rememberCookieSession').checked = true; state.lastCookieProvided = Boolean(clean); persistCrawlerControls(); previewDoubanInput(); setStatus(message); return clean; }
async function importCookieFromClipboard() { try { const text = await navigator.clipboard.readText(); const clean = setCookieBoxValue(text, '已从剪贴板提取 Cookie，并自动启用本次会话记忆。'); if (!clean) setStatus('剪贴板里没有可识别的 Cookie。'); } catch (error) { setStatus('剪贴板读取失败：请手动粘贴完整 Request Headers，系统会自动提取 Cookie。'); } }
...
function renderCookieImportAssistant() { return `<div class="cookie-import-assistant anti-overflow"><b>Cookie 快速导入</b><span class="hint">粘贴完整 Request Headers 会自动提取 Cookie；也可以先在开发者工具里复制 Cookie，再点下面按钮一键读取剪贴板 Cookie。导入后会自动启用本次会话记忆，只保存在 sessionStorage。</span><div class="quick-actions"><button class="ghost" onclick="importCookieFromClipboard()">一键读取剪贴板 Cookie</button><button class="ghost" onclick="setCookieBoxValue($('doubanCookie')?.value || '', '已整理 Cookie 格式，并自动启用本次会话记忆。')">整理 Cookie 格式</button></div><span class="hint">不会读取浏览器密码、历史记录或个人资料；只处理你粘贴/复制到剪贴板的 Cookie 文本。</span></div>`; }
function renderCookieGuide() { return `<details><summary>Cookie 教程</summary><ol class="mini-list"><li>打开浏览器并登录豆瓣。</li><li>进入 https://movie.douban.com/。</li><li>按 F12 打开开发者工具，进入 Network / 网络。</li><li>刷新页面，点任意 movie.douban.com 或 www.douban.com 请求。</li><li>在 Headers / 标头里找到 Request Headers。</li><li>复制 Cookie: 后面的整段内容，粘贴到这里；如果复制了完整 Request Headers，系统会自动抽取 Cookie。</li></ol><p class="hint">Cookie 只用于本机请求豆瓣页面，不会保存到磁盘，也不会出现在推荐报告里。</p></details>`; }
...
function renderCrawlerPanel() {
  ...
  <label>Cookie（可选）</label><textarea id="doubanCookie" oninput="persistCrawlerControls()" onblur="normalizeCookieBox(); persistCrawlerControls(); previewDoubanInput()" placeholder="如果出现 403 / 登录跳转，把浏览器请求里的 Cookie 或完整 Request Headers 粘贴到这里；系统会自动提取 Cookie"></textarea>
