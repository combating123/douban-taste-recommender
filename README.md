...
- Cookie **仅 sessionStorage/请求内存** 使用，且 sessionStorage 仅限当前标签页；同步后输入框会清空或保留在当前标签会话里。
...
遇到这种情况，将你已有的 Cookie 字符串手动粘贴到同步页的 Cookie 输入框后重试。同步请求发出后 Cookie 输入框会自动清空，项目不会把 Cookie 写入磁盘、日志、缓存或推荐报告。
...
3. 点击“开始自动同步”。
4. 需要继续未完成任务时，在同一个标签页的相同输入框再次粘贴后继续。
